...
def data_notebook() -> dict:
    cells = [
        ...
        code_cell(
            """
from datasets.sample_table import list_episode_frame_files, resolve_active_episode_ids

active_episode_ids = resolve_active_episode_ids()
sample_files = list_episode_frame_files(EPISODE_FRAMES_ROOT, active_episode_ids=active_episode_ids)
missing_exports = [episode_id for episode_id in active_episode_ids if not (EPISODE_FRAMES_ROOT / episode_id / "frames.jsonl").exists()]

print("Active bag folders:", len(active_episode_ids))
print("Matched exported episode files:", len(sample_files))
if missing_exports:
    print("Bags without exported frames:", len(missing_exports))
    for episode_id in missing_exports[:10]:
        print("  missing export:", episode_id)
