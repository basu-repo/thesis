...
class ObstacleDetectionNode(Node):
    ...
    def __init__(
        ...
        front_min_range_filter_epsilon: float = 0.0,
        front_min_range_side_margin: float = 0.20,
        ...
        self.front_min_range_filter_epsilon = max(0.0, float(front_min_range_filter_epsilon))
        self.front_min_range_side_margin = max(0.0, float(front_min_range_side_margin))
    ...
    def scan_cb(self, msg: LaserScan):
        ...
        scan_front_min = self._sector_min(msg, -self.front_half_angle, self.front_half_angle)
        scan_left_min = self._sector_min(msg, self.front_half_angle, self.side_angle)
        scan_right_min = self._sector_min(msg, -self.side_angle, -self.front_half_angle)
        front_min = scan_front_min
        front_min_from_scan = True
        left_min = scan_left_min
        right_min = scan_right_min
        ...
        # Very small planar-scan returns right at the sensor minimum range are often
        # self / near-ground artifacts. When the point cloud does not confirm a close
        # frontal block and the side scan still shows space, prefer the 3D reading.
        if (
            self.front_min_range_filter_epsilon > 0.0
            and math.isfinite(scan_front_min)
            and scan_front_min <= (self.min_valid_range + self.front_min_range_filter_epsilon)
        ):
            pointcloud_confirms_close_front = (
                math.isfinite(pc_front_min)
                and pc_front_min <= (self.min_valid_range + self.front_min_range_filter_epsilon + self.front_min_range_side_margin)
            )
            side_scan_indicates_space = (
                (not math.isfinite(scan_left_min) or scan_left_min > scan_front_min + self.front_min_range_side_margin)
                or (not math.isfinite(scan_right_min) or scan_right_min > scan_front_min + self.front_min_range_side_margin)
            )
            if not pointcloud_confirms_close_front and side_scan_indicates_space:
                front_min = pc_front_min if math.isfinite(pc_front_min) else float("inf")
                front_min_from_scan = False
