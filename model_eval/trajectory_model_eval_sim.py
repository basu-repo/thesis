...
from omnet_uav_state_bridge import OmnetUavStateBridge, PROFILES as OMNET_STATE_PROFILES
from project_paths import GUI_CONFIG_PATH, MODELS_DIR, OMNET_DIR, OMNET_EXTERNAL_DIR, RVIZ_CONFIG_PATH, WORLD_SDF_PATH
from resource_usage_monitor import RunResourceMonitor
...
UAV1_X, UAV1_Y, UAV1_Z = 106.035, -271.38, 11.00
UAV2_X, UAV2_Y, UAV2_Z = 107.369, -280.957, 11.00
WORLD_SHARED_GOAL = (-35.0, -290.30, 0.35)
GROUND_MARKER_Z = 0.2025


def yaw_toward_goal(spawn_xyz: tuple[float, float, float], world_goal: tuple[float, float, float]) -> float:
    return math.atan2(float(world_goal[1]) - float(spawn_xyz[1]), float(world_goal[0]) - float(spawn_xyz[0]))
...
HUSKY2_SPAWN_YAW = yaw_toward_goal((HUSKY2_X, HUSKY2_Y, HUSKY2_Z), WORLD_SHARED_GOAL)
# Pinned to the manually adjusted pose shown in Gazebo.
UAV1_SPAWN_YAW = -3.036460
UAV2_SPAWN_YAW = -3.033500
HUSKY2_SPAWN_QZ, HUSKY2_SPAWN_QW = yaw_to_quat(HUSKY2_SPAWN_YAW)
UAV1_SPAWN_QZ, UAV1_SPAWN_QW = yaw_to_quat(UAV1_SPAWN_YAW)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--omnet-config",
        default="WifiRelay",
        help="OMNeT configuration name. Local basic configs include WifiRelay and BluetoothRelay; external configs may use names such as Communication-GazeboBridge-WiFi/5G/LoRa.",
    )
...
def build_omnet_command(config_name: str) -> str:
    if config_name in {"WifiRelay", "BluetoothRelay"}:
        return (
            f"cd {OMNET_DIR / 'onmetpp'} && "
            f"./onmetpp -u Cmdenv -f omnetpp.ini -c {config_name}"
        )
...
def local_omnet_state_profile(config_name: str):
    name = str(config_name).strip().lower()
    if name == "wifirelay":
        return OMNET_STATE_PROFILES["wifi"]
    if name == "bluetoothrelay":
        return OMNET_STATE_PROFILES["bluetooth"]
    return None
...
def main() -> int:
    ...
    for name, x, y, z, qz, qw in (
        ("uav1", UAV1_X, UAV1_Y, UAV1_Z, UAV1_SPAWN_QZ, UAV1_SPAWN_QW),
        ("uav2", UAV2_X, UAV2_Y, UAV2_Z, UAV2_SPAWN_QZ, UAV2_SPAWN_QW),
    ):
    ...
    local_omnet_profile = local_omnet_state_profile(args.omnet_config) if args.enable_omnet else None
    omnet = None
    if args.enable_omnet and local_omnet_profile is None:
        log_event("Starting external OMNeT communication co-simulation...")
        omnet = run_bg(build_omnet_command(args.omnet_config))
        time.sleep(3)
    elif args.enable_omnet:
        log_event(
            "Using local communication relay profile "
            f"{local_omnet_profile.name}: delay={local_omnet_profile.fixed_delay_s:.3f}s "
            f"jitter={local_omnet_profile.jitter_s:.3f}s drop={local_omnet_profile.drop_probability:.3f}"
        )
    ...
    resource_monitor = RunResourceMonitor(
        node_name="run_resource_monitor_09",
        controller_state_topic="/husky_2/controller_state",
        clock_topic="/clock",
    ...
    omnet_uav_state_bridge = None
    impaired_world_pose_topic = f"/world/{WORLD_NAME}/dynamic_pose/info"
    impaired_uav1_odom_topic = "/model/uav1/odometry"
    impaired_uav2_odom_topic = "/model/uav2/odometry"
    if args.enable_omnet and local_omnet_profile is None:
        ...
        impaired_world_pose_topic = f"/omnet/world/{WORLD_NAME}/dynamic_pose/info"
        impaired_uav1_odom_topic = "/omnet/model/uav1/odometry"
        impaired_uav2_odom_topic = "/omnet/model/uav2/odometry"
        omnet_uav_state_bridge = OmnetUavStateBridge(
            node_name="omnet_uav_state_bridge",
            world_pose_input_topic=f"/world/{WORLD_NAME}/dynamic_pose/info",
            world_pose_output_topic=impaired_world_pose_topic,
            husky_model_name="husky_2",
            uav1_input_odom_topic="/model/uav1/odometry",
            uav2_input_odom_topic="/model/uav2/odometry",
            uav1_output_odom_topic=impaired_uav1_odom_topic,
            uav2_output_odom_topic=impaired_uav2_odom_topic,
            topic_prefix="/omnet",
            fixed_delay_s=local_omnet_profile.fixed_delay_s,
            jitter_s=local_omnet_profile.jitter_s,
            drop_probability=local_omnet_profile.drop_probability,
            base_rssi_dbm=local_omnet_profile.base_rssi_dbm,
            base_snir_db=local_omnet_profile.base_snir_db,
        )
    ...
    obstacle_detector = ObstacleDetectionNode(
        ...
        front_min_range_filter_epsilon=0.04,
        front_min_range_side_margin=0.25,
    )
    model_driver = HuskyTrajectoryModelDriver(
        ...
        uav1_odom_topic=impaired_uav1_odom_topic,
        uav2_odom_topic=impaired_uav2_odom_topic,
        world_pose_topic=impaired_world_pose_topic,
    ...
    if omnet_uav_state_bridge is not None:
        managed_nodes.append(omnet_uav_state_bridge)
