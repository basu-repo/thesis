...
class HuskyTrajectoryModelDriver(Node):
    ...
    def _safety_override(self) -> tuple[bool, str]:
    ...
    def step(self):
        ...
        safety_active, safety_state = self._safety_override()
        if safety_active:
        ...
        if bootstrap:
            ...
            linear = clamp(self.cmd_linear_gain * max(target_distance, 0.0), 0.0, self.max_linear_speed)
            if abs(heading) > self.heading_slowdown_threshold:
                linear *= 0.45
