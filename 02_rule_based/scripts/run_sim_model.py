...
# 4.5 m is easier for both UAVs to hold than the previous 6.0 m in Baylands.
UAV_INITIAL_FORWARD_OFFSET = 0.0
UAV_SIDE_OFFSET = 4.5

# High initial altitude to reduce tree/canopy collisions.
UAV_INITIAL_HEIGHT = 22.0

# The UAV controller later rotates UAVs to the Husky heading.
# Spawn yaw only affects initial pose.
...
# Test goal in the opposite direction.
# Original long Baylands goal was:
RAW_WORLD_SHARED_GOAL = (-54.4904, -96.9701, -0.2737)
GOAL_WORLD_PULLBACK = 13.0
# RAW_WORLD_SHARED_GOAL = (130.0, 70.0, -0.2737)
...
# Faster UAV catch-up without changing Husky behavior.
UAV_MAX_XY_SPEED = 12.0
UAV_MAX_Z_SPEED = 2.0
...
def write_uav_variant(
    output_path: Path,
    robot_namespace: str,
) -> Path:
    """Create a UAV SDF variant with the correct robotNamespace.

    The base m100/model.sdf may contain robotNamespace=uav1.
    For two UAVs, each spawned model must get its own namespace so that uav1
    and uav2 do not accidentally share internal plugin topics.
    """
    sdf_text = (MODELS_DIR / "m100" / "model.sdf").read_text()

    # Replace all occurrences, not only the first one.
...
def spawn_uav(
    *,
    ...
    """Spawn a UAV from a UAV-specific SDF variant."""
    spawn_cmd = """
ign service -s /world/{world_name}/create \
--reqtype ignition.msgs.EntityFactory \
--reptype ignition.msgs.Boolean \
--timeout 5000 \
--req 'sdf_filename: "{sdf_path}", name: "{name}",
    ...
    if include_uav1:
        topics.extend(
            [
                ...
                # UAV1 command topics for dataset/exporter command features.
                "/model/uav1/command/twist",
                "/uav1/command/twist",

                # UAV1 perception/state.
    ...
    if include_uav2:
        topics.extend(
            [
                ...
                # UAV2 command topics for dataset/exporter command features.
                "/model/uav2/command/twist",
                "/uav2/command/twist",

                # UAV2 perception/state.
...
# Create UAV-specific SDF variants before spawning.
# This keeps m100/model.sdf unchanged and avoids uav2 accidentally using uav1 namespace.
uav1_sdf_path = write_uav_variant(
    MODELS_DIR / "m100" / "model_uav1.sdf",
    "uav1",
)

uav2_sdf_path = write_uav_variant(
    MODELS_DIR / "m100" / "model_uav2.sdf",
    "uav2",
)

log_event(f"UAV1 SDF variant: {uav1_sdf_path}")
log_event(f"UAV2 SDF variant: {uav2_sdf_path}")
...
log_event("UAV namespace fix: uav1 and uav2 are spawned from separate SDF variants")
log_event("UAV bag fix: UAV command topics are recorded for both uav1 and uav2")
log_event("UAV formation fix: lateral offset reduced to 4.5 m, faster XY catch-up, no smoothing")
log_event("==============================")
