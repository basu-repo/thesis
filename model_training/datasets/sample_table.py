...
from datasets.paths import RAW_BAGS_ROOT


def resolve_active_episode_ids(raw_bags_root: Path | None = None) -> list[str]:
    bags_root = Path(raw_bags_root) if raw_bags_root is not None else RAW_BAGS_ROOT
    if not bags_root.exists():
        return []
    return sorted(path.name for path in bags_root.iterdir() if path.is_dir())


def list_episode_frame_files(
    episode_frames_root: Path,
    active_episode_ids: list[str] | None = None,
) -> list[Path]:
    if active_episode_ids is None:
        active_episode_ids = resolve_active_episode_ids()
    if active_episode_ids:
        frame_files = []
        for episode_id in active_episode_ids:
            frames_path = episode_frames_root / episode_id / "frames.jsonl"
            if frames_path.exists():
                frame_files.append(frames_path)
        return frame_files
    return sorted(episode_frames_root.glob("*/frames.jsonl"))
...
def load_episode_streams(
    ...
    frame_files = list_episode_frame_files(episode_frames_root, active_episode_ids=active_episode_ids)
