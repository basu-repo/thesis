...
class HazardMapBuilderNode(Node):
    ...
    def _compute_guidance(self) -> tuple[str, list[float]]:
        ...
        if guidance == "clear" and blocked_reports:
            nearest = min(blocked_reports, key=lambda report: report["distance"])
            guidance = f"obstacle_front distance={nearest['distance']:.2f}"

        if guidance.startswith("obstacle_front") and now < self.terrain_sure_until and not local_blocked:
            nearest_distance = min((report["distance"] for report in blocked_reports), default=999.0)
            if nearest_distance > self.terrain_sure_break_distance_m and not any(
                abs(report["lateral"]) <= 1.0 for report in blocked_reports
            ):
                guidance = f"terrain_sure_front distance={self.terrain_sure_distance_m:.2f}"
                terrain_sure_latched = True
