...
GUI_CONFIG = RULE_BASED_ROOT.parent / "01_simulation_world" / "gui" / "baylands_gui.config"
...
UAV_X, UAV_Y, UAV_Z = -197.8070, -162.5520, 9.8197
...
UAV_SPAWN_YAW = yaw_toward_goal((UAV_X, UAV_Y, UAV_Z), WORLD_UAV_GOAL)
UAV2_SPAWN_YAW = yaw_toward_goal((UAV2_X, UAV2_Y, UAV2_Z), WORLD_UAV_GOAL)
...
gz = run_bg(f"ign gazebo --gui-config {GUI_CONFIG} {WORLD}")
time.sleep(5)
...
try:
    ...
    managed_nodes = [episode_metadata]
    if obstacle_detector is not None:
